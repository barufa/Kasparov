...
void Floyd_Warshall(int V){//O(|V|^3)
	forn(i,V)forn(j,V)dist[i][j]=((i==j)? 0:INF);
	forn(i,V)forn(j,V)path[i][j]=i;
	forn(i,V)forall(j,G[i]){dist[i][j->nodo]=j->peso;path[i][j->nodo]=i;}
	forn(k,V)forn(i,V)if(dist[i][k]!=INF)forn(j,V)if(dist[k][j]!=INF)
