...
typedef struct SegTreeLazy{
	...
	tipo &operator[](int idx){return T[sz+idx+1];}
	tipo crt(tipo a[],int l,int r,int pos){//Funcion Interna
		if(l==r)T[pos]=a[r];
		else T[pos]=oper(crt(a,l,(l+r)/2,2*pos),crt(a,(l+r)/2+1,r,2*pos+1));
		return T[pos];
	}
	void create(tipo arr[],int n,tipo ntr,tipo (*fnc) (tipo,tipo)){//O(nlgn)
	...
	tipo query(int ql, int qr, int pos, int l, int r){//O(lgn)
		if(qr<=l || ql>=r) return neutro;
		push(pos,l,r);
		if(ql<=l&&r<=qr)return T[pos];
		return oper(query(ql, qr, 2*pos, l, (l+r)/2), query(ql, qr, 2*pos+1, (l+r)/2, r));
	}
	tipo query(int l, int r){return query(l,r+1,1,0,sz-1);}
	...
	void update(int l, int r,Alt val){update(val,l,r+1,1,0,sz-1);}
	void clear(){T.clear();A.clear();}
}SegTreeLazy;
